...
export default function ContactIcons() {
  ...
  return (
    <section id="contact" className="py-20 px-6 text-center bg-gray-800">
      <h2 className="text-3xl font-bold mb-12 text-blue-400 tracking-wide drop-shadow-lg">📬 Contact</h2>
      <div className="flex flex-wrap justify-center gap-4">
        {contacts.map((c,i)=>(
          <a key={i} href={c.link} target="_blank" rel="noopener noreferrer" className={`flex items-center justify-center w-16 h-16 rounded-full text-white ${c.color} transition`}>
            {c.icon}
          </a>
        ))}
      </div>
